...
contract LockToken is Initializable, OwnableUpgradeSafe, PausableUpgradeSafe {
    ...
    */
    ...
    returns (uint256 _id)
    {
        ...
        if( feesInETH > 0 ) {
            uint256 minAmountMultiple = feesInETH.mul(10 ** 18).mul(_amounts.length);
            require(msg.value >= minAmountMultiple, "Low fee amount");
            uint256 feeDiff = msg.value - minAmountMultiple;
            (bool success,) = companyWallet.call.value(minAmountMultiple)("");
            require(success, "Fee transfer failed");
            /* refund difference. */
            if (feeDiff > 0) {
                (bool refundSuccess,) = _msgSender().call.value(feeDiff)("");
        ...
        for(i=0; i<_amounts.length; i++){
            ...
            allDepositIds.push(_id);
            depositsByWithdrawalAddress[_withdrawalAddress].push(_id);
            emit LogLocking(_id, _withdrawalAddress, _amounts[i]);
        }
    }
